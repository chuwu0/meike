                ...
                if( $scope.treatStatus==1){//主请客人第一次下单，主请客人追单，被请客人追单
                    ...
                    $http.post(UrlConstant.url + "/order/shopCart.do", {json:JSON.stringify({
                        ...
                        if($scope.nowBaby.length>0){
                            alert("购物车中有宝贝");
                            if($scope.nowBaby[0].place=="其他" || $scope.nowBaby[0].place.indexOf($scope.address)>=0){// 妹子的地址之前选的宝贝与现在所选地址相符合
                                alert("地址符合条件，保留宝贝");
                                $scope.hasMei=true;
                                $scope.beInvited=false;
...
// **********************选择包厢*************************
        ...
        $scope.addressChoose=function(){
            if(getElem("placeName")=="其他"){//妹子场地不限
                storeElem("type",2);
                location.href="search.html";
            }else{//妹子有固定场地
                storeElem("type",2);
                location.href="searchLimit.html";
            }
        };
...
// **********************去请客*********************
        $scope.$on('ngRepeatFinished', function(ngRepeatFinishedEvent) {
            $timeout(function(){
                ...
                if(priceChooseList.length>0){
                    ...
                    var widthD=dotList[0].offsetWidth;
        ...
        $scope.invitedChoose=function () {
            location.href="babyList.html?inviteAddress="+$scope.address+"&userId="+$scope.userId+"&treatStatus="+$scope.treatStatus;
        };
