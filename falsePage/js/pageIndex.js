$(function(){
    ...
    setupWebViewJavascriptBridge(function (bridge){
        ...
        $(document).on('touchend', '.yue div',function() {
          var _thisId = Number($(this).attr('babyId'));
    ...
    var timer =  setInterval(function () {
      if(ios.treatStatus){
        ...
        //妹子状态判断
        function isHaveOrder(haveOrder){
          if(haveOrder === 3){
            return 'yue'
          }else{
            return 'have-yue'
        ...
        //当前页数，总页数
        var tab1Page = 0,tab2Page = 0,pageCount = 10,downTime = 0;
                ...
                success: function(res){
                  ...
                  if(tab1Page <= pageCount+1){
                    for(var i = 0; i < data.length;i++){
                      result += '<li>'+
                                '<a href="'+ detailUrl +'?userId=' + theRequest.userId +'&babyId='+ data[i].id + isHaveAddress() +'">'+
                                   ...
                                   '</div> '+
                                '</a>'+
                                '<div babyId="'+ data[i].id +'" class="'+ isHaveOrder(data[i].haveOrder) +'"><div>'+ haveString(data[i].haveOrder) +'</div></div>'+
                            '</li>';
